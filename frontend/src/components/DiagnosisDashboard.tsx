...
// Fix TS1484: Use type-only import for verbatimModuleSyntax
...
const severityMap = {
	0: {
		...
		border: "border-green-500",
		bg: "bg-green-500/10",
		cardHighlight: "bg-green-500/20 border-green-500 shadow-lg shadow-green-500/30 scale-[1.02]",
	},
	1: {
		label: "1 - Trace",
		color: "text-yellow-400",
		border: "border-yellow-500",
		bg: "bg-yellow-500/10",
		cardHighlight: "bg-yellow-500/20 border-yellow-500 shadow-lg shadow-yellow-500/30 scale-[1.02]",
	},
	2: {
		label: "2 - Mild",
		color: "text-orange-400",
		border: "border-orange-500",
		bg: "bg-orange-500/10",
		cardHighlight: "bg-orange-500/20 border-orange-500 shadow-lg shadow-orange-500/30 scale-[1.02]",
	},
	3: {
		label: "3 - Severe",
		color: "text-red-400",
		border: "border-red-500",
		bg: "bg-red-500/10",
		cardHighlight: "bg-red-500/20 border-red-500 shadow-lg shadow-red-500/30 scale-[1.02]",
	},
};
...
>>>>>>> c2bee55f43271fb2a4ede6381f29257ddf9ae9b6
...
=======
	...
	return (
		/* Tailwind v4 Upgrade: bg-linear-to-br */
		<div className="min-h-screen text-white flex items-center justify-center p-8 bg-linear-to-br from-slate-900 to-gray-900">
			<div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-3 gap-6">
				<div className="lg:col-span-2 space-y-6">
					<div className="glass rounded-3xl p-6 bg-white/5 backdrop-blur-md border border-white/10">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-slate-300 mt-1">Atrial Fibrillation Severity Analysis</p>
							</div>
							<div className={`px-6 py-3 rounded-2xl border ${severityInfo.bg} ${severityInfo.border}`}>
						...
						<div className="glass rounded-3xl p-6 bg-white/5 backdrop-blur-md border border-white/10">
							<div className="flex items-center justify-between mb-4">
								<div>
									...
									<p className="text-slate-400 text-sm">Softmax Probability</p>
								</div>
								<div className="text-right">
									<p className="text-4xl font-bold text-cyan-400">{data.confidence}%</p>
								</div>
							</div>
							<div className="h-72 flex items-center justify-center">
								<canvas ref={chartRef}></canvas>
						...
						<div className="glass rounded-3xl p-6 bg-white/5 backdrop-blur-md border border-white/10">
							<div className="flex items-center justify-between mb-4">
								<div>
									...
									<p className="text-slate-400 text-sm">Calculated AF Burden</p>
								</div>
								<div className="text-right">
									<p className="text-4xl font-bold text-emerald-400">{data.burden}%</p>
								</div>
							</div>
							<div className="mt-10">
								<div className="w-full h-8 bg-slate-800 rounded-full overflow-hidden">
									{/* Tailwind v4 Upgrade: bg-linear-to-r */}
									<div
										className="h-full bg-linear-to-r from-emerald-400 to-green-500 rounded-full flex items-center justify-end pr-4 font-semibold transition-all duration-500"
					...
					<div className="glass rounded-3xl p-6 bg-white/5 backdrop-blur-md border border-white/10">
						<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
							<div>
								...
								<p className="text-slate-400 text-sm">Runtime diagnostics</p>
							</div>
							<div className="flex gap-4 flex-wrap">
								{/* Tailwind v4 Upgrade: min-w-45 */}
								<div className="bg-slate-900/70 border border-slate-700 rounded-2xl px-5 py-4 min-w-45">
									<p className="text-slate-400 text-sm">Hardware Used</p>
									<div className="flex items-center gap-2 mt-1">
										<span className="w-3 h-3 rounded-full bg-green-400"></span>
								...
								<button
									onClick={onReset}
									className="bg-cyan-500 hover:bg-cyan-400 text-slate-900 rounded-2xl px-6 py-4 font-bold transition-colors"
								>
									NEW SCAN
								</button>
							</div>
						...
						{[0, 1, 2, 3].map((level) => {
							...
							return (
								<div
									key={level}
									className={`p-4 rounded-2xl border transition-all duration-300 ${
										isActive ? current.cardHighlight : "bg-slate-800 border-slate-700 opacity-50"
									}`}
								>
									<div className="flex justify-between items-center">
